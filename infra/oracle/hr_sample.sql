-- =====================================================================
-- HR 샘플 스키마 (raw_schema 역할)
--
-- DataGrip / DBeaver 같은 GUI 클라이언트에서 스크립트 전체를 그대로 실행할 수 있습니다.
-- SQL*Plus 전용 명령(@@ 인클루드, ACCEPT, SET, PROMPT, SPOOL)을 모두 제거했습니다.
--
--   접속   : system 등 CREATE USER 권한이 있는 계정으로 FREEPDB1 에 접속
--   실행   : 이 파일 전체를 스크립트로 실행 (DataGrip: Execute Script / DBeaver: Alt+X)
--   결과   : HR 사용자 + 테이블 7개 + EMPLOYEES 107행
--   계정   : hr / hr_dev_2026
--
-- 원본은 infra/oracle/human_resources/ 의 Oracle 공식 스크립트입니다.
-- 이 파일은 그것을 합쳐서 만든 것이며, 컨테이너 초기화도 같은 파일을 씁니다.
-- =====================================================================

-- 이미 있으면 지우고 다시 만듭니다 (재실행 가능).
DECLARE
...
CREATE USER hr IDENTIFIED BY "hr_dev_2026"
...
-- 이후 생성되는 객체는 HR 스키마에 들어갑니다.
ALTER SESSION SET CURRENT_SCHEMA=HR;
-- hr_populate 의 날짜 리터럴이 미국 형식이라 필요합니다.
